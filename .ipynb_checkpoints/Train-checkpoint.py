...
def main():
    ...
    out = '/Users/sukhdevchaman/Desktop/slfproject/out'
    ...
    for step in range(start_step, steps):
        ...
        for group in optimizer.param_groups:
            group['lr'] = lr_at(step, lr, warmup_steps,
        ...
        if (step + 1) % log_every == 0:
            ...
            print('step {:>7}/{}  loss {:.5f}  lr {:.2e}  H(mask) {:.3f}/{:.3f}  '
                  'slots_used {}/{}  recon [{:+.2f},{:+.2f}]  {:.0f} ms/step'.format(
                      step + 1, steps, mean_loss, lr, entropy, max_entropy,
                      slots_used, num_slots, r_min, r_max, sec * 1000))
            writer.writerow([step + 1, mean_loss, lr, entropy, slots_used,
                             r_min, r_max, sec])
            log_file.flush()
        ...
        if (step + 1) % ckpt_every == 0:
            save(step, loss_value, 'last.pt')

    save(steps - 1, loss_value, 'last.pt')
    log_file.close()
    print('\nDone. best loss {:.5f}. Checkpoints in {}'.format(best_loss, out))
    print('Next: python eval.py --ckpt {}'.format(os.path.join(out, 'best.pt')))
