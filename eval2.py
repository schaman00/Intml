...
def ari_foreground(gt_masks, pred_masks):
    b = gt_masks.shape[0]
    true_fg = gt_masks[:, 1:].flatten(2).permute(0, 2, 1).float()   

    k = pred_masks.shape[1]
    winner = pred_masks.squeeze(-1).argmax(dim=1).flatten(1)       
    pred_onehot = F.one_hot(winner, k).float()                      
...
def main():
    ...
    if not checkpoint_paths:
        raise FileNotFoundError(
            f'Nessun checkpoint .pt trovato in: {checkpoint_dir}'
        )

    print(f'Trovati {len(checkpoint_paths)} checkpoint')
    ...
    for checkpoint_path in checkpoint_paths:
        ...
        print(f'\nValuto checkpoint: {checkpoint_path}')
        ...
        print(
            '{:<25} MSE {:.5f}   ARI-FG {:.4f}   ({} val images)'.format(
                checkpoint_name,
                mse,
                ari,
                n
        ...
        qualitative_path = os.path.join(
            out_dir,
            f'qualitative_{checkpoint_name}.png'
        )
    ...
    print('\nCheckpoint migliore secondo ARI-FG:')
    print(f'  modello: {best_name}')
    print(f'  path:    {best_result["checkpoint"]}')
